...
提示：  
(1) 略  
(2) 恒成立求参数取值范围，先考虑分离参数。本题的 $a$ 容易分离，得到：

$$
a \leq \frac{e^{2x}-2xe^x-1}{x^3}
$$
当 $x = 0$ 时，右边的函数值是 $\frac{0}{0}$ 未定式。如果求导讨论，最后也会发现是单调递增的，最小值是 $x=0$ 处的极限值。因此本题只能按部就班地分类讨论。构造函数 $g(x)=f(x)-ax^3-1$，求导得 $g'(x) = 2e^{2x} - 2(x+1)e^x - 3ax^2 - 1$ ，性质不明，继续求导 $g''(x) = 4e^{2x} - 2(x+2)e^x - 6ax$，性质不明，继续求导 $g'''(x) = 8e^{2x} - 2(x+3)e^x - 6a$，性质不明，继续求导 $g''''(x) = 2e^x(8e^x-x-4)$ ，其中 $8e^x - x - 4$ 性质不明，继续求导 $8e^x - 1$，则 $g''''(x) \geq 2e^x(3\ln 2 - 3)$ 